...
DLLEXPORT char* startposeservice(char* str,bool isshow=false, int x = -1, int y = -1, int width = -1, int height = -1)
{
	std::string svofile(str);

	char* resbuffer = (char*)zedservice::startposeservice(svofile,isshow,x,y,width,height).data();
	return resbuffer;
}
