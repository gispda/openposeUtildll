...
std::string openposeUtil::startmergereportavi(std::string hmavi_file, std::string zedsvo_file, std::string zedposeavi_file, std::string posedata_dir)
{
	...
	while (poseavicap.read(zedposeimg)&& hkcap.read(simg1))
	{
		...
		fromjson(posedata_dir+std::to_string(i)+".json");
		cout << "over from json" << endl;
		//recalcposedatimg(jsonbodypose);
		//cv::imshow("Test2", img2);
		//waitKey(10);
		cv::resize(zedposedataimg, img4, zedposeimg.size());
		cout << "----------------------" << endl;
...
void openposeUtil::fromjson(std::string json_file)
{
			...
			if (need_new_image) {
				...
				//	sl::sleep_ms(2);
					...
					if (!image_ocv.empty())
					{
						...
						//if (bsaveZed) {
						//
						//	cv::resize(inputImage, rrimg, cv::Size(image_width, image_height));
