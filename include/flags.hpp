...
// Debugging/Other
DEFINE_int32(logging_level,             0,              "The logging level. Integer in the range [0, 255]. 0 will output any opLog() message,"
...
DEFINE_string(openpose_root_dir, "d:/devtools/openpose/", "Process a directory of images. Use `examples/media/` for our default example folder with 20"
...
DEFINE_string(camera_resolution,        "-1x-1",        "Set the camera resolution (either `--camera` or `--flir_camera`). `-1x-1` will use the"
                                                        ...
                                                        " `--flir_camera`");
DEFINE_string(video,                    "examples/media/video.avi",             "Use a video file instead of the camera. Use `examples/media/video.avi` for our default"
                                                        " example video.");
...
DEFINE_string(prototxt_path,            "pose_deploy.prototxt",             "The combination `--model_folder` + `--prototxt_path` represents the whole path to the"
...
DEFINE_int32(display,                   3,             "Display mode: -1 for automatic selection; 0 for no display (useful if there is no X server"
...
DEFINE_string(grid_number_inner_corners, "9x6", "Number of inner corners in width and height, i.e., number of total squares in width"
	" and height minus 1.");
// Mode 1 - Intrinsics
DEFINE_string(camera_serial_number, "20854143", "Camera serial number.");
// Mode 2 - Extrinsics
...
DEFINE_string(camera_parameter_folder, "models/cameraParameters/flir/", "String with the folder where the camera parameters are or will be"
	" located.");
DEFINE_string(camera_parameter_dir, "C:/ProgramData/Stereolabs/settings/", "String with the folder where the camera parameters are or will be"
	" located.");
